...
class ConfigurableDistiller(BaseDistiller):
    ...
    def compute_distill_losses(self, data_samples) -> LossResults:
        ...
        for loss_name, forward_mappings in self.loss_forward_mappings.items():
            ...
            if hasattr(loss_module, 'with_inputs') and loss_module.with_inputs:
                loss = loss_module(inputs=data_samples, **forward_kwargs)  # type: ignore
            else:
