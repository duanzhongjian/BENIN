...
def train_single_fold(cfg, num_splits, fold, resume_ckpt=None):
    ...
    vis_interval = cfg.default_hooks.visualization.interval
    if vis_interval > num_splits:
        cfg.default_hooks.visualization.interval = int(vis_interval // num_splits)
    ...
    if dataset.type == 'RepeatDataset':
        ori_dataset = copy.deepcopy(train_dataset.dataset)
        kfold_dataset = wrap_dataset(ori_dataset, False)
        train_dataset.dataset = kfold_dataset
    ...
    if cfg.val_dataloader is not None:
        ...
        if dataset.type == 'RepeatDataset':
            val_dataset = copy.deepcopy(dataset.dataset)
        else:
    ...
    if cfg.test_dataloader is not None:
        if 'pipeline' not in cfg.test_dataloader.dataset:
            raise ValueError(
                ...
                'Please write config files manually.')
        if dataset.type == 'RepeatDataset':
            test_dataset = copy.deepcopy(dataset.dataset)
        else:
    ...
    if dataset.type == 'RepeatDataset':
        runner.logger.info(f'Train dataset: \n{runner.train_dataloader.dataset.dataset}')
    else:
