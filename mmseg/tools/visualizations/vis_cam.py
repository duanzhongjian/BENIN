...
from mmseg.models.utils.encoder_decoder import Encoder_Decoder
...
def main():
    ...
    gt_mask = np.float32(data['data_samples'].gt_sem_seg.data.cpu().numpy())[0]

    mmcv.imwrite(gt_mask * 255, str(args.save_path) + 'gt.png')
    mmcv.imwrite(pred_mask * 255, str(args.save_path) + 'pred.png')
    print('pred mask saved to {}'.format('output/pred.png'))
