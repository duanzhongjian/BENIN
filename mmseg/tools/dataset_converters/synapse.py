...
def open_nii_file(file_path):
    file = np.rot90(nib.load(file_path).get_fdata())
    file = np.rot90(file)
    file = np.rot90(file)
    return file
...
def pares_args():
    ...
    parser.add_argument(
        '--direction', default=2, type=int, help='3D image slice direction.')
    parser.add_argument(
        '--p',
        default=1,
        type=float,
        help='the probability of extracting images')
    parser.add_argument(
        '--save-path',
...
def main():
    ...
    if osp.exists(osp.join(dataset_path, 'train.txt')) \
        ...
        train_imgs_id = read_files_from_txt(
            osp.join(dataset_path, 'train.txt'))

        val_imgs_id = read_files_from_txt(osp.join(dataset_path, 'val.txt'))

        train_imgs_id = [
            id_.replace('img', '').replace('.nii.gz', '')
            for id_ in train_imgs_id
        ]

        val_imgs_id = [
            id_.replace('img', '').replace('.nii.gz', '')
            for id_ in val_imgs_id
        ]
        ...
        progress_bar = ProgressBar(len(train_imgs_id))
        ...
        for i, img_id in enumerate(train_imgs_id):
            if osp.exists(osp.join(
                    ...
                    'img',
                    'img' + img_id + '.nii.gz')) \
                    and osp.exists(osp.join(
                    dataset_path,
                    'label',
                    'label' + img_id + '.nii.gz')):

                img_nii = open_nii_file(
                    osp.join(dataset_path, 'img', 'img' + img_id + '.nii.gz'))
                label_nii = open_nii_file(
                    osp.join(dataset_path, 'label',
                             'label' + img_id + '.nii.gz'))

                imgs, masks = split_image(img_nii, label_nii, args.direction,
                                          args.p, args.drop_background)
                ...
                for j, (img, mask) in enumerate(zip(imgs, masks)):
                    ...
                    if np.max(mask) >= 14:
                        print(img_id)
                    img.save(
                        osp.join(save_path, 'img_dir', 'train',
                                 save_name + '.jpg'))
                    mask.save(
                        osp.join(save_path, 'ann_dir', 'train',
                                 save_name + '.png'))
                progress_bar.update()
            else:
                continue
        ...
        for i, img_id in enumerate(val_imgs_id):
            if osp.exists(osp.join(
                ...
                for j, (img, mask) in enumerate(zip(imgs, masks)):
                    ...
                    img.save(
                        osp.join(save_path, 'img_dir', 'val',
                                 save_name + '.jpg'))
                    mask.save(
                        osp.join(save_path, 'ann_dir', 'val',
                                 save_name + '.png'))
            else:
                continue
    else:
        ...
        for i, img_id in enumerate(train_imgs_id):
            if osp.exists(osp.join(
                ...
                for j, (img, mask) in enumerate(zip(imgs, masks)):
                    ...
                    img.save(
                        osp.join(save_path, 'img_dir', save_name + '.jpg'))
                    mask.save(
                        osp.join(save_path, 'ann_dir', save_name + '.png'))
            else:
                continue
