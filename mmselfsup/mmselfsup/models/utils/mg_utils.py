...
def nonlinear_transformation(x, prob=0.5):
    ...
    nonlinear_x = np.interp(x.cpu(), xvals, yvals)
    nonlinear_x = torch.tensor(nonlinear_x, device='cuda')
    return nonlinear_x

def local_pixel_shuffling(x, prob=0.5):
    if random.random() >= prob:
        return x
    image_temp = copy.deepcopy(x.cpu())
    orig_image = copy.deepcopy(x.cpu())
    ...
    return torch.tensor(local_shuffling_x, device='cuda')
...
def generate_pair(img, batch_size, config, status="test"):
    ...
    while True:
        ...
        if config.save_samples is not None and status == "train" and random.random() < 0.01:
            ...
            sample_1 = np.concatenate((x[n_sample,0,:,:], y[n_sample,0,:,:]), axis=1)
            sample_2 = np.concatenate((x[n_sample,0,:,:], y[n_sample,0,:,:]), axis=1)
            sample_3 = np.concatenate((x[n_sample,0,:,:], y[n_sample,0,:,:]), axis=1)
            sample_4 = np.concatenate((x[n_sample,0,:,:], y[n_sample,0,:,:]), axis=1)
